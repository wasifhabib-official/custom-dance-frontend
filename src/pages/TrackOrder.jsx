  ...
  const handleResponse = async (token, type, message = "") => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(
          type === "APPROVE"
            ? "Approved! Moving to the next step."
            : "Changes sent to Stacy."
        );
      }
    } catch (err) {
      ...
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-extrabold text-gray-900 tracking-tight font-heading uppercase">
            Track Your <span className="text-rose-600 font-light">Order</span>
          </h1>
        ...
        {error && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="flex items-center gap-3 p-5 bg-rose-50 text-rose-700 rounded-2xl border border-rose-100 mb-6"
          >
            <AlertCircle size={20} />
            ...
            >
                  ...
                  {STEPS.map((step) => {
                    ...
                    return (
                      <div key={step.key} className="relative pl-12 pb-10 last:pb-0">
                        <div
                          className={`absolute left-0 top-1 w-8 h-8 rounded-full z-10 flex items-center justify-center transition-all ${
                            isCompleted
                              ? "bg-green-500 shadow-lg shadow-green-100 scale-110"
              ...
              {actions.length > 0 && (
                <div className="bg-[#1e293b] rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
                  ...
                  <p className="text-slate-400 text-xs mb-8">
                    Review the files below and let us know if we're ready to stitch.
                  </p>
                  ...
                  {actions.map((action) => (
                    ...
                    >
                      <p className="font-black text-white mb-6 uppercase tracking-widest text-[10px]">
                        Proof for: {action.type.replace(/([A-Z])/g, " $1")}
                      </p>
              ...
              <div className="text-center p-10 bg-white/50 rounded-[2rem] border border-gray-50">
                ...
                <div className="flex justify-center gap-8">
                  <Link
                    to="/contact"
                    className="text-gray-900 font-black text-xs uppercase tracking-widest border-b-2 border-rose-200 hover:border-rose-500 transition-all pb-1"
                  >
                    Contact Stacy
                  </Link>
                  <Link
